...
- **Batch results are validated and repaired.** In 0.13 batch results were never validated after a resume. In 1.0 every `map` item is validated against `schema`; items that fail go through the realtime repair pass (`repair.attempts`, default 1), which costs a realtime call per failed item. A WARN naming the batch id is logged when more than half of a batch fails validation — usually a schema the provider's structured-output mode cannot express (nested discriminated unions on Gemini). The Google batch path now sends the full JSON Schema (`responseJsonSchema`) for that reason.
...
- **Batch accounting rows** now store the item prompt and `durationMs` in `metadata`, like realtime rows.
