...
- Reworked the async-batch subsystem onto the AI SDK's batch seam, added OpenRouter's Batch API as a transport, and corrected batch cost accounting.
  ...
  **Breaking (pre-1.0 minor):**
  ...
  - `ModelConfig.batchDiscountPercent` is deprecated in favour of `batchInputCostPerMillion` / `batchOutputCostPerMillion`, which the sync CLI now reads from OpenRouter's catalog. **Re-run `workflow-engine-sync`** — a registry generated by 0.12 has no batch prices and 39 models incorrectly flagged as batch-capable.
  ...
  **Deprecations:**
  ...
  - `ModelConfig.batchDiscountPercent` — use `batchInputCostPerMillion` / `batchOutputCostPerMillion`. Removal at 1.0.
