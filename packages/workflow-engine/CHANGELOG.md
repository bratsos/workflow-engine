...
- Reworked the async-batch subsystem onto the AI SDK's batch seam, added OpenRouter's Batch API as a transport, and corrected batch cost accounting.

  **Breaking (pre-1.0 minor):**

  - Removed the three raw vendor batch provider classes and their config/request types from the public API: `AnthropicBatchProvider`, `GoogleBatchProvider`, `OpenAIBatchProvider`, `AnthropicBatchProviderConfig`, `GoogleBatchProviderConfig`, `OpenAIBatchProviderConfig`, `AnthropicBatchRequest`, `GoogleBatchRequest`, `OpenAIBatchRequest`, `BatchRequestText`, `BatchRequestWithSchema`, and `BatchStatus`. Batch now runs through the AI SDK's batch models (`@ai-sdk/google`, `@ai-sdk/anthropic`, `@ai-sdk/openai`) or through OpenRouter's Batch API. `ai.batch()` itself is unchanged for the common case.
  - Peer dependencies swapped: `@anthropic-ai/sdk`, `@google/genai`, and `openai` are no longer peers. `@ai-sdk/anthropic` (>=4.0.46) and `@ai-sdk/openai` (>=4.0.53) are new optional peers; `@ai-sdk/google` is already a direct dependency. This removes ~20 MB of optional install surface.
  - `ai.batch(modelKey)` now throws when the model has no known batch-capable provider. It previously fell through to `"google"` and failed later with a confusing Google-credentials error.
  - `AIBatchProvider` gained `"openrouter"`. Code that exhaustively `switch`es on it needs a new case.
  - `ModelConfig` gained `batchModelId`, `batchInputCostPerMillion`, and `batchOutputCostPerMillion`, read by the sync CLI from OpenRouter's `:batch` catalog rows. `supportsAsyncBatch` is now true when the catalog has a `:batch` sibling **or** the vendor is google/anthropic/openai (whose AI SDK providers batch natively); `batchDiscountPercent` is kept, scoped to the native transports' documented 50%. **Re-run `workflow-engine-sync`** — a registry generated by 0.12 has no batch prices and its capability flag was a prefix heuristic with 39 false positives.
  - Recorded cost values change. Cost is now taken from the provider when it reports one, and batch cost uses real per-model batch prices instead of a flat 50%. Anything alerting on `WorkflowRun.totalCost` deltas will fire once on upgrade.

  **Bug fixes:**

  - **Two workers could both record the same batch's cost.** `recordResults` guarded with a check-then-act `isRecorded()`; the batch id lived only in `metadata` JSON, so no uniqueness could be enforced. `ai_calls` gains nullable `batchId` / `requestId` columns with a unique index, and `logBatchResults` writes with `skipDuplicates`, so a concurrent second writer is a no-op. **Consumers must add the two columns and the index** (schema and SQL in the migration guide); rows written by earlier versions are still recognised.
  ...
  - **Batch output was silently capped at 1024 tokens** on the Anthropic and OpenAI paths while Google had no cap, so identical structured-output workloads truncated on two providers and succeeded on the third — surfacing as a JSON parse error that never mentioned truncation. `AIBatchRequest` now accepts `maxTokens`, `system`, and `temperature`, defaulting to the model's own `maxCompletionTokens`.
  - **Per-tool observability records were only written when the caller happened to pass `onStepEnd`.** They are now written whenever tools are used.
  - **A cancelled batch polled until its deadline.** Cancelled and expired batches now report `"failed"` with an explanatory `error`.
  - `provider.max_price` was pinned to the exact registry price, so any upstream price drift made calls fail outright rather than cost slightly more. It now carries 25% headroom, configurable via `priceHeadroom`.
  - Long-context pricing tiers from OpenRouter's `pricing.overrides` are now applied. Models that double above a prompt-length threshold (e.g. `anthropic/claude-sonnet-4.5` above 200k tokens) were under-reported by 2x.
  - `workflow-engine-sync` no longer requires `OPENROUTER_API_KEY`; the catalog endpoint is public. It also now sees all 565 catalog models instead of 418, and derives embedding models intrinsically instead of via a second request.

  **New features:**

  - `npx workflow-engine-codemod [--from 0.11|0.12] [--dry-run] [--json] [paths...]` applies the mechanical renames from 0.11→0.12 (`experimental_output`→`output`, `onStepFinish`→`onStepEnd`, `streamText`'s `system`→`instructions`) and reports, with file and line, every 0.13 item that needs a human: removed batch provider imports, `getStatus`/`getResults` called without metadata, `batchDiscountPercent` in hand-written models, `apiKey` in suspended state, and vendor SDK dependencies your own code never imports.
  - `createAIHelper(topic, logger, logContext?, providerResolver?, options?)` gained a fifth parameter, `AIHelperOptions { routing?: OpenRouterRoutingOptions }`, so `priceHeadroom` (default 1.25), `sort`, and `requireParameters` on OpenRouter routing are configurable per helper — including for embeddings — instead of hard-coded. Child helpers inherit it.
  - `BatchOptions.abortSignal` cancels every provider call a batch handle makes (submit, status polls, result fetches).
  - `BatchSubmitError` (exported) is thrown when a later partition of a fan-out fails after earlier batches were already created upstream; it carries `createdRefs` so the surviving batches can be polled and drained instead of orphaned.
  - Batch inspection outside a stage: `resolveAiSdkBatchModel`, `fromAiSdk`, `createOpenRouterBatchModel` and the `EngineBatchModel` / `EngineBatchStatus` / `EngineBatchItemResult` / `EngineBatchRef` types are exported, so an admin page can check a batch through the engine instead of three vendor SDKs.
  - `AIBatchResult.validated?: boolean` reports whether a result was actually checked against its schema. Zod schemas are not JSON-serializable and never survived a suspend/resume, so validation silently did not happen in the production lifecycle — the types claimed otherwise. Re-supply schemas via `getResults(batchId, { schemas })` to validate; a warning is logged when results come back unvalidated.
  - `ai.batch(modelKey, provider?, options?)` accepts `BatchOptions { apiKey, baseURL, fetch, endpoint, maxRequestsPerBatch, maxPartitions }`, so credentials and transport can be injected instead of read from `process.env`. Batch no longer touches `process.env` at construction time, which unblocks edge runtimes.
  - OpenRouter is available as a batch transport (`ai.batch(model, "openrouter")`), which brings batch to the nine model authors that previously had no batch code path at all. Note the transport's limits: text-only, a 24h window, and no partial results on expiry.
  - A `submit()` may now fan out into multiple upstream batches, partitioned by response schema and capped at 500 requests each (`maxRequestsPerBatch`). `AIBatchHandle` gained `refs`, `batchIds`, `requestCounts`, `totalRequests`, and `error`; persist `refs` in `suspendedState.metadata.batchRefs` and pass that metadata to **both** `getStatus()` and `getResults()`. `getStatus(batchId, metadata?)` takes an optional second argument for exactly this reason — without it, it polls only the first batch. `handle.id` still holds the first batch id, so the `suspendedState.batchId` contract is unchanged.
  - `AITextResult`, `AIObjectResult`, `AIEmbedResult`, and the stream's `getUsage()` gained `reportedCostUsd` and `costSource: "reported" | "estimated"`. BYOK is handled correctly: under BYOK the upstream inference cost is added, under non-BYOK it is not.
  - Upgraded to `ai@7.0.87`, `@ai-sdk/provider@4.0.9`, and `@ai-sdk/google@4.0.59`. The `^` ranges already admitted these, so consumers were resolving them while CI tested `7.0.22`.

  **Deprecations:**

  - `SuspendedStateSchema.apiKey` — read by nothing, and it invites persisting a provider key into a `Json` column that round-trips on every poll. Use `BatchOptions.apiKey`. Removal at 1.0.
