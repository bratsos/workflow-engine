...
describe("scoped storage", () => {
  it("saves under the grant prefix and the orchestrator can read it back via BlobStore", async () => {
    ...
    await orchestrator.submit({ workflowRunId: "r1", stageId: "download", stageName: "D", stageNumber: 1, input: {}, config: {}, workflowContext: {}, pollInterval: 100, maxWaitTime: 10_000 });
    const task = await worker.lease({ workerId: "w1", stageIds: ["download"], stageCodeVersion: "v1" });

    const storage = createScopedStorage(task!.grant, worker, task!.taskId, task!.leaseToken);
