...
describe("in-process transport", () => {
  it("routes submit/lease/report/poll and putBytes/getBytes through one broker + store", async () => {
    ...
    const { taskId } = await orchestrator.submit({
      ...
      input: {}, config: {}, workflowContext: {}, pollInterval: 100, maxWaitTime: 10_000,
    });
    const task = await worker.lease({ workerId: "w1", stageIds: ["download"], stageCodeVersion: "v1" });
    expect(task?.taskId).toBe(taskId);

    const url = (await worker.presign({ taskId, leaseToken: task!.leaseToken, relKey: `${task!.grant.prefix}a.bin`, op: "put" })).url;
    await worker.putBytes(url, { n: 1 });
    const getUrl = (await worker.presign({ taskId, leaseToken: task!.leaseToken, relKey: `${task!.grant.prefix}a.bin`, op: "get" })).url;
    expect(await worker.getBytes(getUrl)).toEqual({ n: 1 });

    await worker.report({ taskId, leaseToken: task!.leaseToken, outcome: { kind: "completed", output: { ok: 1 } }, logs: [], annotations: [], progress: [] });
