...
const echo = defineStage({
  ...
  async execute(ctx) { return { output: { echoed: ctx.input.msg } }; },
});
...
describe("createActivityWorker", () => {
  it("processOne leases, runs, and reports a task; returns false when idle", async () => {
    ...
    const { orchestrator, worker: transport } = createInProcessTransport(broker, os);
    const { taskId } = await orchestrator.submit({ workflowRunId: "r1", stageId: "echo", stageName: "Echo", stageNumber: 1, input: { msg: "hi" }, config: {}, workflowContext: {}, pollInterval: 100, maxWaitTime: 10_000 });

    const worker = createActivityWorker({
      registry: new Map([["echo", echo]]), transport, workerId: "w1",
      stageIds: ["echo"], stageCodeVersion: "v1",
    });
    ...
    expect((poll.outcome as { output: { echoed: string } }).output.echoed).toBe("hi");
  });
});
